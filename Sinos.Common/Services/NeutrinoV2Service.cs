...
{
    private readonly ILogger<NeutrinoV2Service> _logger;
    ...
    public NeutrinoV2Service(ILogger<NeutrinoV2Service> logger, SettingService settingService, NeutrinoCommonService commonService)
    {
        this._logger = logger;
    ...
    public async Task<ConvertScoreToTimingResult?> ConvertMusicXmlToTiming(ConvertMusicXmlToTimingOption option)
    {
        ...
        using (var monoLabFile = PipeFile.CreateWriteOnly(FileExtensions.Label))
        {
            ...
            {
                // musicXMLtoLabelを実行
                await this._commonService.Execute(command, args, this.GetNeutrinoWorkingDirectory()).ConfigureAwait(false);
            }
            finally
            {
    ...
    public async Task<EstimateFeaturesResultV2> EstimateFeatures(NeutrinoV2Option option, IProgress<ProgressReport>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        using (var additionalDisposables = new DisposableCollection(5))
        {
            ...
            var args = new ProcessArgumentList()
            {
                // <full_lab_path> <timing_lab_path> <f0_path> <mspec_path> <model_path>
                fullLabFile.Path,
                timingFilePath,
                f0File.Path,
                mspecFile.Path,
                this.GetModelPath(option.ModelInfo.ModelId),
            };
            ...
            if (option.NumberOfParallelInSession is { } numberOfParallelInSession)
                args.Add("-o", numberOfParallelInSession);

            if (option.InferenceMode != null)
                args.Add("-d", (int)option.InferenceMode);

            if (option.StyleShift is { } styleShift)
                args.Add("-k", styleShift);

            if (option.RandomSeed is { } randomSeed)
                args.Add("-r", randomSeed);
            ...
            if (option.WorldFeaturesPrediction)
            {
                ...
                args.Add("-w", mgcFile.Path, bapFile.Path);
            }

            if (option.SinglePhrasePrediction is { } singlePhrasePrediction)
                args.Add("-p", singlePhrasePrediction);

            if (option.UseSingleGpu is { } useSingleGpu)
                args.Add("-g", useSingleGpu);
    ...
    public async Task<byte[]> SynthesisWorld(WorldV2Option option, IProgress<ProgressReport>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        using (var wavFile = PipeFile.CreateWriteOnly(FileExtensions.Wave))
        {
            ...
            var args = new ProcessArgumentList()
            {
                // <f0_path> <mgc_path> <bap_path> <wav_path>
                f0File.Path,
                mgcFile.Path,
                bapFile.Path,
                wavFile.Path,
            ...
            if (option.EnhanceBreathiness is { } enhancedBreathiness)
                args.Add("-b", enhancedBreathiness);
    ...
    public async Task<byte[]> SynthesisNSF(NSFV2Option option, IProgress<ProgressReport>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        using (var additionalDisposable = new DisposableCollection())
        {
            ...
            var args = new ProcessArgumentList
            {
                // <f0_path> <mspec_path> <wav_path> <model_path> <output_wav_path>
                f0File.Path,
                mspecFile.Path,
                $@"./model/{option.Model.ModelId}/{option.ModelType.FileName}",
                wavFile.Path,
            };
